...
{
    var start = (page - 1) * rows;//starting of table content
    var end = start+rows;//end of table content here 10 row per page
    var data = querySet.slice(start,end);//getting only 10 data
    var pages = Math.ceil(querySet.length/rows); //total pages will be 10 as we have 100/10.
    return{
        "querySet":data,
        "pages":pages
    }
...
function pageButtons(pages) {
    ...
    wrapper.innerHTML = ""
    var maxLeft = state.page-2// always we will have 2 buttons in left to current selected button
    var maxRight = maxLeft+4// always we will have 2 buttons in right to current selected button
    console.log("Left :"+maxLeft +" Right :"+maxRight +" Current :"+state.page);
    if (maxLeft < 1) {//may be we can get negative value
        maxLeft = 1
        maxRight = maxLeft+4
    }
    if (maxRight > pages) { // When selecting last page maxRight will be higher than total page
        maxLeft = pages - (state.window - 1) 
        if (maxLeft < 1)
        	maxLeft = 1
        maxRight = pages
    }
    console.log("Left :"+maxLeft +" Right :"+maxRight +" Current :"+state.page);//check whether page button calculation works fine
    for (var page = maxLeft; page <= maxRight; page++) {//create n buttons
    	wrapper.innerHTML += `<button value=${page} class="page btn btn-sm btn-primary">${page}</button>`
    }
    if (state.page != 1) { // create first page &#171 is for '<<' symbol
        wrapper.innerHTML = `<button value=${1} class="page btn btn-sm btn-primary">&#171; First</button>` + wrapper.innerHTML
    }
    if (state.page != pages) { // create last page &#187 is for '>>' symbol
        wrapper.innerHTML += `<button value=${pages} class="page btn btn-sm btn-primary">Last &#187;</button>`
    }
    $('.page').on('click',function(){ // to get the current page data
        $('#table-body').empty();
        state.page=$(this).val();
        buildTable();
    })
...
function buildTable() {
    ...
    {
        //we can also use this instead of template literals
        
        // var row = document.createElement("tr");
        // var tid = document.createElement("td");
        // var tname = document.createElement("td");
        // var temail = document.createElement("td");
        // tid.innerHTML=myList[i].id;
        // tname.innerHTML=myList[i].name;
        // temail.innerHTML=myList[i].email;
        // row.append(tid,tname,temail); 

        //template literals for creating rows
        var row = `<tr> 
                  <td>${myList[i].id}</td>
                  <td>${myList[i].name}</td>
